...
bool svabaAssemblerEngine::performAssembly(int num_assembly_rounds)
{
    ...
    std::vector<std::string> contig_pos;
    ...
    for (int yy = 1; yy < num_assembly_rounds; yy++) {
        ...
        for (auto& j : m_contigs) {
            // improve the EML4-ALK (region) contig assembly, modified by Schaudge King. 2021-08-05!
            contig_pos = split(j.Name, '_');
            if ((contig_pos[1] == "2" && ((stoi(contig_pos[2]) > 29440000 && stoi(contig_pos[2]) < 2945000) ||
                                          (stoi(contig_pos[2]) > 42492000 && stoi(contig_pos[2]) < 42553300))) || j.Seq.length() > m_readlen)
                tmpc.push_back(j);
        }
