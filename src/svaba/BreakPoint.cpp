...
void BreakPoint::splitCoverage(svabaReadVector &bav) {
    ...
    for (auto& i : bav) {
        ...
        if (valid_reads.count(i.SR())) {
            ...
            // check that it's not a bad 1, 2 split, *** will be good for ctDNA, ***
    ...
    for (auto& i : allele) {
        ...
        i.second.adjust_alt_counts((int)reject_qnames.size());
    }
...
void BreakPoint::__combine_with_discordant_cluster(DiscordantClusterMap& dmap) {
    ...
    const int PAD = 100; // 50 for svaba version 1.0.0
    ...
    for (auto& d : dmap) {
        ...
        bool pass = (bp1reg1 || bp2reg2) && s1 && s2; // bp1reg1 && bp2reg2 && s1 && s2 ?
...
void BreakPoint::score_assembly_only() {
    ...
    if (!b1.local && !b2.local && !complex_local) // added this back in v71
        ...
        confidence = "LOWAS";
    else if ( (std::max(b1.nm, b2.nm) >= 3 || std::min(b1.as_frac, b2.as_frac) < 0.80) && getSpan() < 0 ) // !!! 0.85 ---> 0.80
        confidence = "LOWAS";
    else if ((double) aligned_covered/(double) seq.length() < 0.75) // !!! less than 75% of read is covered by some alignment
        confidence = "LOWAS";
    else if ( (b1.matchlen < 50 && b1.mapq < 60) || (b2.matchlen < 50 && b2.mapq < 60) )
        confidence = "LOWMAPQ";
    else if (std::min(b1.nm, b2.nm) >= 10)
        confidence = "LOWMAPQ";
    else if (a.split <= 5 && span <= 1500 && span != -1) // !!! small with little split, add to 5
...
void BreakPoint::addCovs(const std::unordered_map<std::string, STCoverage*>& covs, std::map<std::string, std::string>& bams,
                         std::ofstream* log_file, bool bp1_in = true) {
    ...
    for (auto& i : covs)  {
        ...
        allele[i.first].cov = c / 2 / (COVERAGE_AVG_BUFF*2 + 1);
        // std::max(i.second->getCoverageAtPosition(b1.gr.chr, b1.gr.pos1), i.second->getCoverageAtPosition(b2.gr.chr, b2.gr.pos1));

        // *** Modified By Schaudge King, 2019-08-08, adjust non target region's total coverage computation! ***
        if (allele[i.first].alt > allele[i.first].cov) {
            SeqLib::GenomicRegion connect_region(bp1_in ? b2.gr.chr : b1.gr.chr,
                                                 bp1_in ? b2.gr.pos1 - COVERAGE_AVG_BUFF - 50: b1.gr.pos1 - COVERAGE_AVG_BUFF - 50,
                                                 bp1_in ? b2.gr.pos1 + COVERAGE_AVG_BUFF + 51 : b1.gr.pos1 + COVERAGE_AVG_BUFF + 51);
            (*log_file) << "Adjust break point " + cname + " proper depth (coverage) estimation with new region: " << connect_region << std::endl;
            svabaBamWalker walker1 = svabaBamWalker();
            walker1.Open(bams[i.first]);
            walker1.prefix = "m000";
            set_walker_params(walker1);
            walker1.SetRegion(connect_region);
            walker1.readBam(log_file); // do the reading bam reads records

            auto connect_cov = 0;
            for (int jj = -COVERAGE_AVG_BUFF; jj <= COVERAGE_AVG_BUFF; ++jj) {
                connect_cov +=  walker1.cov.getCoverageAtPosition(b1.gr.chr, b1.gr.pos1 + jj);
                connect_cov +=  walker1.cov.getCoverageAtPosition(b2.gr.chr, b2.gr.pos1 + jj);
            }
            connect_cov = connect_cov / 2 / (COVERAGE_AVG_BUFF*2 + 1);
            walker1.clear();

            if (allele[i.first].alt < connect_cov)
                allele[i.first].cov = connect_cov;
            else
                std::cerr << "This maybe a germline structure variant (over 50% new contigs than refence sequence)!" << std::endl;
        }
...
std::string SampleInfo::toFileString() const {
    ...
    if (indel)
        ss << std::setprecision(4) << genotype << ":" <<
           ...
           << ":" << LO_n << ":" << LO << ":" << ((double) std::max(alt,cigar)/cov < 1.0 ? (double) std::max(alt,cigar)/cov : 1.0); // << ":" << SLO;
    else
        ss << std::setprecision(4) << genotype << ":" <<
           alt << ":" << cov << ":" << GQ << ":" << PL << ":" << split
           << ":" << disc << ":" << LO_n << ":" << LO << ":" << ((double) alt/cov < 1.0 ? (double) alt/cov: 1.0); // << ":" << SLO;
...
void SampleInfo::adjust_alt_counts() {
    ...
    // modified by Schaudge King, improve the alt reads counts, because of under estimate the reads number
    alt = std::max((int)qn.size(), cigar);

}

void SampleInfo::adjust_alt_counts(int reject_reads_sizes) {
    ...
    // alt count is max of cigar or unique qnames (includes split and discordant)
    // modified by Schaudge King, improve the alt reads counts, because of under estimate the reads number
    alt = std::max((int)qn.size() + reject_reads_sizes, cigar);
...
bool ReducedBreakPoint::operator < (const ReducedBreakPoint& bp) const {
    ...
    if (cov > 100 && cov < bp.cov + 100 && quality + 30 < bp.quality)
        return true;
    else if (bp.cov > 100 && cov + 100 > bp.cov && quality > bp.quality + 30)
        return false;
