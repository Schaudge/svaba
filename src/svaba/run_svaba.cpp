...
namespace opt {
    ...
    static int num_to_sample = 2000000;  // num to learn from (e.g. isize distribution)
...
void runsvaba(int argc, char** argv) {
    ...
    // make one anyways, we check if its empty latermicrobe_bwa = nullptr;
...
bool runWorkItem(const SeqLib::GenomicRegion& region, svabaThreadUnit& wu, long unsigned int thread_id) {
    ...
    for (auto& i : dmap) {
        // don't send DSCRD if FR and below size
    ...
    // Minor modification for breakpoint within this region of bam walker
    // Schaudge King, 2019-08-08
    for (auto& i : bp_glob) {
        bool bp1_in_region = (i.b1.gr.chr == region.chr && i.b1.gr.pos1 > region.pos1 - 100 && i.b1.gr.pos1 < region.pos2 + 100) ? true : false;
        i.addCovs(covs, opt::bam, & log_file, bp1_in_region);
    }
    ...
    for (const auto& a : alc)
        wu.m_bamreads_count += a.NumBamReads();
    for (auto& i : bp_glob)
        if ((i.b1.chr_name == "2" && i.b2.chr_name == "2" && ((i.b1.gr.pos1 > 29440000 && i.b1.gr.pos1 < 29450000 &&
                                                               i.b2.gr.pos1 > 42492000 && i.b2.gr.pos1 < 42553300) ||
                                                              (i.b1.gr.pos1 > 42492000 && i.b1.gr.pos1 < 42553300 && i.b2.gr.pos1 > 29440000 && i.b2.gr.pos1 < 29450000))) ||
            (i.hasMinimal() && (i.confidence != "NOLOCAL" || i.complex_local)) )
            wu.m_bps.push_back(i);

    // dump if getting too much memory
...
void sendThreads(SeqLib::GRC& regions_torun) {
    ...
    for (int i = 0; i < opt::numThreads; ++i) {
        ...
        // Memory "Leak_DefinitelyLost" was detected by Valgrind-3.13.0, the followings was modified by Schaudge King.
...
void run_assembly(const SeqLib::GenomicRegion& region, svabaReadVector& bav_this, std::vector<AlignedContig>& master_alc,
    ...
    if (!region.IsEmpty()) {
        try {
            ...
            WRITELOG(" Caught exception for lregion with region " + region.ToString(bwa_header), true, true);
            lregion = "";
        }
    }
    ...
    for (auto& i : all_contigs_this) {
        ...
        // if too short, skip
        if (i.Seq.length() < readlen && !opt::all_contigs)
            continue;
        ...
        // add in the chromosome name tag for human alignments
        if (main_bwa)
