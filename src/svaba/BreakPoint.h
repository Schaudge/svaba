...
struct SampleInfo {
    ...
    void adjust_alt_counts(int reject_reads_sizes);
...
struct BreakPoint {
    ...
    // keep track of reads to reject, *** not proper for ctDNA, comment by Schaudge King. ***
    std::set<std::string> reject_qnames; // tumor ctDNA reads small than 150bp
    ...
    void addCovs(const std::unordered_map<std::string, STCoverage*>& covs, std::map<std::string, std::string>& bams, std::ofstream* log_file, bool bp1_in);
