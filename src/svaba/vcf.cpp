...
static std::string sv_format = "GT:AD:DP:GQ:PL:SR:DR:LR:LO:AF";
static std::string indel_format = "GT:AD:DP:GQ:PL:SR:CR:LR:LO:AF";
...
std::ostream& operator<<(std::ostream& out, const VCFEntry& v) {
    ...
    vector<pair<string, std::string> > tmpvec; // id, everything else
...
VCFFile::VCFFile(std::string file, std::string id, const SeqLib::BamHeader& h, const VCFHeader& vheader, bool nopass) {
    ...
    sv_header.addFormatField("AF","1","Float","The estimated fraction of the variant");
...
// deduplicate for potential same rearrangement event
void VCFFile::deduplicate() {
    ...
    for (auto& i : entry_pairs) {
        ...
        std::unordered_map<int, std::pair<size_t, size_t>> key_count; // left and right is paired
        ...
        for (auto& j : key_count) {
            ...
            if (j.second.first && j.second.second) { // left and right hit for this key
                if (*entry_pairs[j.first]->bp < *i.second->bp) { // this has worst read coverage that what it overlaps, so mark as dup. If tie, take left-most break
                    // check that it's not a local clashing with a global, because they're supposed to be two annotations for one event
